...
* [第29回領域セミナ・第81回DLAP：　野村 悠祐 氏 「機械学習手法を用いた量子多体問題の解析」](#第29回学習物理領域セミナー第81回dlap) 7/9
...
[講演スライド](./slides/「学習物理学の創成」領域セミナー.pdf)<br>
...
発表者： 野村 悠祐 氏 (東北大学)<br>
場所：ZOOM（オンラインのみ）<br>
発表題目：機械学習手法を用いた量子多体問題の解析<br>
[講演スライド](./slides/presentation_nomura_2026_07_09.pdf)<br>
概要：物質が示す多様な物性や量子相の性質を根源的に記述する量子多体問題は、基礎科学における最も重要かつ困難な課題の一つです。
近年、人工ニューラルネットワークを用いた量子状態表現が、この課題に取り組むための有望な新しいアプローチとして注目を集めています。
本講演では、以下の3つの課題について議論します。<br>
(i) 良い変分アンサッツをどのように構築するか<br>
(ii) どのように統一的な精度評価を行うか<br>
(iii) 実際の物質（実材料）へどのように適用するか<br>
まず、(i)については、物理的知見を取り入れることで精度向上を目指すアプローチ[1] や、最適化手法を工夫しニューラルネットワークを大規模化することで精度向上を目指すアプローチ[2] を紹介します。
次に、(ii)については、エネルギー分散の期待値に基づき、異なる変分波動関数や異なるハミルトニアンに対する変分法の精度を統一的に評価する指標「Vスコア」の導入について紹介します[3]。これは、古典コンピュータを用いた変分法の最前線を可視化するとともに、量子コンピュータが目指すべきターゲットを明確にするものです。
最後に、(iii)については、この手法を物質固有の問題へどのように適用できるかを示し、すでに有望な成果が得られつつあることを紹介します[4]。
さらに、時間が許せば、深層ボルツマンマシンによる解析的な基底状態のマッピングや、有限温度計算への拡張についても議論します。<br>
参考文献<br>
[1] 例えば，Y. Nomura et al., Phys. Rev. B 96, 205152 (2017).<br>
[2] 例えば，A. Chen and M. Heyl, Nat. Phys. 20, 1476 (2024).<br>
[3] D. Wu et al., Science 386, 296 (2024).<br>
[4] レビューとして，例えば Y. Nomura and M. Imada, J. Phys. Soc. Jpn. 94, 031001 (2025).<br>
