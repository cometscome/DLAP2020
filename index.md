...
* [第30回領域セミナ・第82回DLAP：　松原 崇 氏 「データの幾何学と深層学習　～画像やテキストの構造を見つける～」](#第30回学習物理領域セミナー第82回dlap) 7/23
* [第29回領域セミナ・第81回DLAP：　野村 悠祐 氏 「TBA」](#第29回学習物理領域セミナー第81回dlap) 7/9
* [第28回領域セミナ・第80回DLAP：　野尻 美保子 氏 「科学計算コード生成におけるLLMエージェントのタスク分解と監査」](#第28回学習物理領域セミナー第80回dlap) 6/25
...
# 第30回学習物理領域セミナー＋第82回DLAP
日時：7月23日10:30~11:30(JST)<br>
発表者： 松原 崇 氏 (北海道大学)<br>
場所：ZOOM（オンラインのみ）<br>
発表題目：データの幾何学と深層学習　～画像やテキストの構造を見つける～<br>
概要：深層学習ブームの火付け役だった畳み込みニューラルネットワークは，画像中の物体を，その位置に関わらず認識することができます．この性質は並進群という群に対する同変性という幾何学の言葉で表すことができます．今日注目を浴びる大規模言語モデルでも，それぞれの単語や画像は埋め込み（分散表現）という数値列に変換されます．それらをベクトルとみなして加減算すると，意味の足し引きができると言われています．これは群の直積とみることもできます．このように，深層学習を上手く機能させるには，データに内在する幾何学的な構造への洞察が欠かせません．では，このような知見は画像やテキストの検索や生成にどう活かせるでしょうか？本講演では「データの幾何学」をキーワードに，一見異なるこれらの研究を俯瞰的に紹介します．<br>

# 第29回学習物理領域セミナー＋第81回DLAP
...
# 第28回学習物理領域セミナー＋第80回DLAP
日時：6月25日10:30~11:30(JST)<br>
発表者： 野尻 美保子 氏 (KEK)<br>
場所：ZOOM（オンラインのみ）<br>
発表題目：科学計算コード生成におけるLLMエージェントのタスク分解と監査<br>
概要：近年、LLM は単なる対話型のコード生成器にとどまらず、設計、実装、実行、検査を担う AI エージェント的な部品として使われ始めている。科学計算でもこの方向は有望であるが、得られた数値だけでなく、生成されたコードが何をどのように計算しているかを確認できることが不可欠である。<br>
本講演では、ユーザー仕様から科学計算コードを生成する過程を、仕様の整理、曖昧性の事前監査、補助関数の選択、コード生成、生成コードの意味的トレース、仕様に基づく批判的検査という複数のタスクに分解する枠組みを紹介する。対象例として、高エネルギー物理解析コードを生成する CoLLM-lite を用いる。<br>
タスクを分割することで、従来は 70B 程度の大規模モデルを用いていた処理の一部を、14B 程度の複数のモデルで実行できる。また、各段階の中間出力を人間が監査できるため、生成失敗の原因、仕様の曖昧性、生成コードの意味的なずれを分離して調べることができる。実用的には、このような枠組みは、レガシーライブラリを利用するユーザーや、大規模共同研究で共有された解析資源を再利用する場合にも有用である。LLM にすべてを自由生成させるのではなく、既存資源の選択、組み合わせ、生成後の監査を明示的なタスクとして分けることで、小型モデルの安定利用だけでなく、大規模 LLM を用いる際のタスク設計にも示唆を与える。<br>
...
発表者： 宮戸 岳 氏 (University of Tübingen)<br>
場所：ZOOM（オンラインのみ）<br>
発表題目：物理に着想を得たモデルによる、長期的かつコヒーレントな再帰システムの探究<br>
[講演スライド](./slides/MachineLearningPhysics2026.pdf)<br>
概要：近年のニューラルアーキテクチャの発展により、固定された深さのフィードフォワード計算を超えて計算を拡張する仕組みとして、再帰性への関心が再び高まっている。Chain-of-Thought reasoning はその代表例の一つであり、モデルに追加の計算過程を与えることで、現在のAIの推論能力を大きく押し上げてきた。一方で、long-horizon task、継続的に学習・更新される記憶システム、そして長時間にわたって一貫性を保つ内部状態の設計は、依然として重要な未解決課題である。本講演では、実世界の多くの現象に見られる再帰的な構造、とりわけ物理に着想を得た再帰モデルが、長時間スケールの汎化、潜在状態の組織化、そして永続的なニューラル計算を考えるための有望な言語を与えることを論じる。<br>
特に本講演では自分の携わった三つの関連する方向性に焦点を当てる。第一に、蔵本型の振動子ダイナミクスに基づく潜在再帰モデルである AKOrN について議論する。このモデルでは、ニューラルトークンが超球面上の相互作用する振動子として発展する。この定式化は、同期、位相関係、エネルギーに類するダイナミクスを手がかりに、再帰的計算を組織化する方法を示している。第二に、これらの考えを、永続的な fast–slow recurrent model におけるTrain-Short/Test-Long 汎化へと接続する。この枠組みでは、潜在状態は各観測ごとにリセットされるのではなく、複数回の更新によって反復的に洗練され、ストリーム全体にわたって持ち越される。第三に、ニューラルネットワークにおける自発的対称性の破れという視点を導入する。ここでは、ネットワークに明示的に U(1) 対称性を組み込むことにより、Goldstone モードに類似した永続的な自由度が生じうることを考える。これは、従来の edge of chaos のように臨界点への微調整によって情報伝播を保つのとは異なり、自発的対称性の破れた相において、層方向あるいは再帰時間方向に長距離の情報伝播を支える可能性を与える。<br>
